...
import { LayoutDashboard, Briefcase, Activity, LifeBuoy, AlertTriangle, RefreshCw, Star, Download, Receipt, Settings } from 'lucide-react'
...
const WIDGETS = [
  { key: 'cases', label: '案件總數' },
  { key: 'active', label: '進行中案件' },
  { key: 'tickets', label: '待處理工單' },
  { key: 'overdue', label: '逾期生產' },
  { key: 'satisfaction', label: '平均滿意度' },
  { key: 'outstanding', label: '未收款項' },
  { key: 'stages', label: '案件階段分佈' },
  { key: 'activity', label: '最近活動' },
]

function StatCard({ icon, label, value, tone = 'slate' }) {
  const tones = { slate: 'bg-slate-50 text-slate-600', indigo: 'bg-indigo-50 text-indigo-600', amber: 'bg-amber-50 text-amber-600', rose: 'bg-rose-50 text-rose-600' }
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-3">
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${tones[tone]}`}>{icon}</div>
      <div className="min-w-0"><div className="text-2xl font-bold text-slate-800 leading-none">{value}</div><div className="text-xs text-slate-400 mt-1">{label}</div></div>
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  const exportCases = () => {
    ...
    const rows = cases.map(c => [c.code, c.title, customerName[c.customer_id] || '', STAGE_MAP[c.stage]?.label || c.stage, STATUS_MAP[c.status]?.label || c.status, profilesMap[c.owner_id] || '', (c.created_at || '').slice(0, 10)])
    downloadCSV(`案件清單_${todayStr()}.csv`, [header, ...rows])
  }

  const statDefs = [
    { key: 'cases', icon: <Briefcase className="w-5 h-5"/>, label: '案件總數', value: cases.length, tone: 'slate' },
    { key: 'active', icon: <Activity className="w-5 h-5"/>, label: '進行中案件', value: activeCases.length, tone: 'indigo' },
    { key: 'tickets', icon: <LifeBuoy className="w-5 h-5"/>, label: '待處理工單', value: openTickets.length, tone: 'amber' },
    { key: 'overdue', icon: <AlertTriangle className="w-5 h-5"/>, label: '逾期生產項目', value: overdueProd.length, tone: 'rose' },
    { key: 'satisfaction', icon: <Star className="w-5 h-5"/>, label: '平均滿意度', value: avgSat ? `${avgSat.toFixed(1)}★` : '—', tone: 'amber' },
    { key: 'outstanding', icon: <Receipt className="w-5 h-5"/>, label: '未收款項 (NT$)', value: num(outstanding), tone: 'rose' },
  ]
  const visibleStats = statDefs.filter(s => show(s.key))
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button onClick={() => setCustomizing(v => !v)} className="inline-flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 hover:bg-white"><Settings className="w-4 h-4"/>自訂</button>
            {customizing && (
              <>
                <div className="fixed inset-0 z-20" onClick={() => setCustomizing(false)}/>
                <div className="absolute right-0 mt-1 w-52 bg-white border border-slate-200 rounded-xl shadow-xl z-30 p-2">
                  <p className="text-xs font-semibold text-slate-400 px-2 py-1">顯示項目</p>
                  {WIDGETS.map(w => (
                    <label key={w.key} className="flex items-center gap-2 px-2 py-1.5 text-sm text-slate-700 hover:bg-slate-50 rounded cursor-pointer">
                      <input type="checkbox" checked={show(w.key)} onChange={() => toggle(w.key)} className="accent-indigo-600"/>{w.label}
                    </label>
                  ))}
                </div>
              </>
      ...
      {visibleStats.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {visibleStats.map(s => <StatCard key={s.key} icon={s.icon} label={s.label} value={s.value} tone={s.tone}/>)}
        </div>
      )}
          ...
          <ul className="divide-y divide-slate-50">
            {events.map(e => (
              <li key={e.id} className="py-2.5 flex items-center justify-between gap-3 text-sm">
                <div className="min-w-0"><span className="text-slate-700">{e.summary || e.type}</span><Link to={`/cases/${e.case_id}`} className="ml-2 text-xs text-indigo-600 hover:underline">{caseTitle[e.case_id] || '案件'}</Link></div>
